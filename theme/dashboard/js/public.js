...
Tickee.calcPlan = function() {
	...
	Public.get("module.calculate").innerHTML = "<input type='text' placeholder='100' />Avg tickets per event<br /><input type='text' placeholder='1' id='input-month' /><span id='descr-month'>event per month</span><br /><input type='text' value='0.5' />service fee<br /><a href='javascript:Tickee.resultPlan()' class='calc'>Calculate</a>"
...
Tickee.resultPlan = function() {
	...
	tpe = form[0].value | form[0].readAttribute( "placeholder" );
	epm = form[1].value | form[1].readAttribute( "placeholder" );
	plan = ( tpe > 2500 )? 250 : 25;
	
	result = ( tpe * epm * form[2].value - plan ) * 12;
